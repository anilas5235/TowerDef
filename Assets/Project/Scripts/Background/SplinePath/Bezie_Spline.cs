...
namespace Background.SplinePath
{
    public class Bezie_Spline : BaseSplineBuilder
    {
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < splinePoints.Count - 1; i += 3)
            {
                UnityEditor.Handles.DrawLine(splinePoints[i].position, splinePoints[i + 1].position, 5f);
                UnityEditor.Handles.DrawLine(splinePoints[i + 2].position, splinePoints[i + 3].position, 5f);
            }
#endif
        }
    }
